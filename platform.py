...
class UnihikerPlatform(PlatformBase):
    ...
    def configure_upstream_arduino_packages(self, url_items):
        ...
        if url_items.scheme == "file":
            local_framework_path = url_items.path
            if IS_WINDOWS and local_framework_path.startswith("/"):
                local_framework_path = local_framework_path[1:]  # Remove leading slash on Windows
            ...
            if os.path.isfile(framework_index_file):
                print("Configuring toolchain packages from local source: %s" % local_framework_path)
                with open(framework_index_file) as fp:
                    self.configure_arduino_toolchains(json.load(fp))
                return
            else:
                print("Warning: Local framework index file not found: %s" % framework_index_file)
                return
        
        # Handle remote GitHub repositories
        framework_index_file = os.path.join(
            self.get_package_dir("framework-arduinounihiker") or "",
            "package",
            "package_esp32_index.template.json",
        )
        ...
        if os.path.isfile(framework_index_file) and os.path.isdir(
            os.path.join(
                self.get_package_dir("framework-arduinounihiker") or "", ".git"
            )
        ):
            with open(framework_index_file) as fp:
                self.configure_arduino_toolchains(json.load(fp))
